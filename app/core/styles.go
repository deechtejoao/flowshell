...
var (
	ImgToggleRight    rl.Texture2D
	ImgToggleDown     rl.Texture2D
	ImgDropdownUp     rl.Texture2D
	ImgDropdownDown   rl.Texture2D
	ImgPushpin        rl.Texture2D
	ImgPushpinOutline rl.Texture2D
	ImgRetry          rl.Texture2D
	ImgPlay           rl.Texture2D
)

// Shared Resources (Fonts)
var (
	InterSemibold uint16
	JetBrainsMono uint16
	InterBold     uint16
	InterRegular  uint16
	CodeFont      uint16
	LogoFont      uint16
...
// UI State Globals (Moved from ui.go)

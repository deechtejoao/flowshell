...
func ExtractChartData(n *Node, xCol, yCol string, chartType ChartType) *ChartRenderData {
...
func NewLineChartNode() *Node {
	return &Node{
		Name:        "Line Chart",
		InputPorts:  []NodePort{{Name: "Data", Type: FlowType{Kind: FSKindTable}}},
		OutputPorts: []NodePort{{Name: "Data", Type: FlowType{Kind: FSKindTable}}},
		Action:      &LineChartAction{},
	}
}

var _ NodeAction = &LineChartAction{}

func (c *LineChartAction) Serialize(s *Serializer) bool {
	SStr(s, &c.XColumn)
	SStr(s, &c.YColumn)
	return s.Ok()
}

func (c *LineChartAction) UpdateAndValidate(n *Node) {
	n.Valid = true
...
func (c *LineChartAction) UI(n *Node) {
	renderData := ExtractChartData(n, c.XColumn, c.YColumn, ChartTypeLine)

	clay.CLAY(clay.IDI("LineChartNode", n.ID), clay.EL{
		Layout: clay.LAY{LayoutDirection: clay.TopToBottom, Sizing: clay.Sizing{Width: clay.SizingFixed(400)}, ChildGap: S2},
	}, func() {
		clay.CLAY(clay.IDI("Settings", n.ID), clay.EL{Layout: clay.LAY{Sizing: GROWH, ChildGap: S2, ChildAlignment: YCENTER}}, func() {
			clay.TEXT("X:", clay.TextElementConfig{TextColor: LightGray, FontID: InterRegular})
			UITextBox(clay.IDI("XCol", n.ID), &c.XColumn, UITextBoxConfig{El: clay.EL{Layout: clay.LAY{Sizing: clay.Sizing{Width: clay.SizingFixed(80)}}}, OnSubmit: func(s string) { c.XColumn = s }})
			clay.TEXT("Y:", clay.TextElementConfig{TextColor: LightGray, FontID: InterRegular})
			UITextBox(clay.IDI("YCol", n.ID), &c.YColumn, UITextBoxConfig{El: clay.EL{Layout: clay.LAY{Sizing: clay.Sizing{Width: clay.SizingFixed(80)}}}, OnSubmit: func(s string) { c.YColumn = s }})
		})
		clay.CLAY(clay.IDI("ChartArea", n.ID), clay.EL{
			...
			Border:          clay.B{Color: Gray, Width: clay.BorderWidth{Left: 1, Right: 1, Top: 1, Bottom: 1}},
...
func (c *LineChartAction) Run(n *Node) <-chan NodeActionResult {
...
var _ NodeAction = &BarChartAction{}

func (c *BarChartAction) Serialize(s *Serializer) bool {
	SStr(s, &c.XColumn)
	SStr(s, &c.YColumn)
	return s.Ok()
}

func (c *BarChartAction) UpdateAndValidate(n *Node) {
	n.Valid = true
	if w, ok := n.GetInputWire(0); ok {
		n.OutputPorts[0].Type = w.Type()
	} else {
...
func (c *BarChartAction) UI(n *Node) {
...
func (c *BarChartAction) Run(n *Node) <-chan NodeActionResult {
	done := make(chan NodeActionResult)
	go func() {
		defer close(done)
		val, _, err := n.GetInputValue(0)
...
var _ NodeAction = &ScatterPlotAction{}

func (c *ScatterPlotAction) Serialize(s *Serializer) bool {
	SStr(s, &c.XColumn)
	SStr(s, &c.YColumn)
	return s.Ok()
}

func (c *ScatterPlotAction) UpdateAndValidate(n *Node) {
	n.Valid = true
	if w, ok := n.GetInputWire(0); ok {
		n.OutputPorts[0].Type = w.Type()
	} else {
...
func (c *ScatterPlotAction) UI(n *Node) {
...
func (c *ScatterPlotAction) Run(n *Node) <-chan NodeActionResult {
	done := make(chan NodeActionResult)
	go func() {
		defer close(done)
		val, _, err := n.GetInputValue(0)
